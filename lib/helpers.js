...
function setProperties(el, attrs){
  ...
  for(let attr in attrs){
    if(attr in el){ // 주어진 속성이 표준 속성인 경우
      el[attr] = attrs[attr];
    }else{
...
function setContents(el, contents){
  ...
  contents.forEach(function(content){
    if(content instanceof Node){ // content가 Node 타입(DOM Element)인 경우 appendChild 함수를 사용할 수 있다
      el.appendChild(content);
    }else if(typeof content === "string") {
      el.innerText = content; // content가 string인 경우
    }
  })
}
...
function readElementProp(id, prop){
  ...
  // data-url 같은 비표준 속성은 첫번째 if문처럼 가져올수 없기 때문에 아래와 같이 함
